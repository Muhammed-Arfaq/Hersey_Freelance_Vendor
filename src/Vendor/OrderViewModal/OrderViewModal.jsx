...
import { orderModalOff } from "../../Redux/Reducer/viewOrderModal";

export default function OrderViewModal() {
    const cancelButtonRef = useRef(null);
    const dispatch = useDispatch()
    const show = useSelector((state) => state.viewOrderDetails.show)
    const data = useSelector((state) => state.viewOrderDetails.data)
    console.log(data);
                                        ...
                                        <div class="border-0 rounded-md ">
                                            <div class="md:flex px-4 leading-none max-w-4xl">
                                                ...
                                                <div class="flex-col ">
                                                    ...
                                                    <p class="hidden md:block px-4 my-4 text-sm text-left"><span className="font-semibold mr-3">User:</span>{data?.userId?.fullName}</p>

                                                    <p class="hidden md:block px-4 my-4 text-sm text-left"><span className="font-semibold mr-3">Contact:</span>{data?.userId?.phone}</p>
                                                    ...
                                                    <p class="hidden md:block px-4 my-4 text-sm text-left"><span className="font-semibold mr-3">Location:</span>{data?.userId?.location}</p>
